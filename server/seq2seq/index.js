...
let Translator = function () {
    ...
    this.loadMetadata = (metadata) => {
        ...
        const translationMetadata = JSON.parse(fs.readFileSync(metadata));
    ...
    this.loadModel = (model_path, metadata_path) => new Promise((resolve, reject) => {
        let self = this;
        tf.loadLayersModel(model_path)
            .then(model => {
                model.summary();
                self.loadMetadata(metadata_path);
                resolve(model);
            })
            .catch(error => {
                console.error(error)
